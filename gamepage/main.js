...
let players = [
    { x: 100, y: 150, width: 50, height: 50, color: "blue", keys: { up: 38, down: 40, left: 37, right: 39 } },
    { x: 400, y: 150, width: 50, height: 50, color: "red", keys: { up: 87, down: 83, left: 65, right: 68 } }
];
...
function movePlayers() {
    players.forEach((player) => {
        ...
        if (keysPressed[player.keys.right] && player.x < canvas.width - player.width) player.x += 5;
        if (keysPressed[player.keys.up] && player.y > 0) player.y -= 5;
        if (keysPressed[player.keys.down] && player.y < canvas.height - player.height) player.y += 5;
    });
}
